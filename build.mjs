...
import {renderScenes,scenes,familyScene,photoSequence} from './scenes.js';
import {renderCrack} from './crack-scroll.js';
import {renderCare} from './care-story.js';
...
for(const file of ['care-story.css','scroll-frame.js','scroll-motion.css','refinement.css','paths.js','site-config.js','crack-scroll.js','crack-scroll.css','styles.css','story.css','story.js','motion.js','motion.css','scenes.js','commerce.css','catalog.js','commerce.js','app.js'])await fs.copyFile(path.join(root,file),path.join(out,file));
...
function localized(lang,route){let html=source.replace('<div id="scene-story-root"></div>',renderScenes(lang)).replace('<div id="crack-scroll-root"></div>',renderCrack(lang)).replace('<div id="gallery-sharing-root"></div>',photoSequence(familyScene,lang)).replace('<div id="gallery-road-root"></div>',photoSequence(scenes[0],lang)).replace('<div id="gallery-gaming-root"></div>',photoSequence(scenes[1],lang)).replace('<html lang="ar" dir="rtl">',`<html lang="${lang}" dir="${lang==='ar'?'rtl':'ltr'}">`);
html=html.replace('<div id="care-story-root"></div>',renderCare(lang));
