...
import {renderHomepageUgc,skuPostImages} from './ugc-posts.js';
...
const requiredImages=[...products.flatMap(p=>['cup','case'].map(f=>packImage(p,f))),...scenes.map(s=>s.image),...skuPostImages,...['whole','husk','kernel','bag'].map(id=>`/assets/ritual-${id==='whole'?'whole-edge-v5':id+'-v4'}.webp`)];
await Promise.all(requiredImages.map(file=>fs.access(path.join(root,file.replace(/^\//,'')))));
await fs.mkdir(path.join(out,'assets'),{recursive:true});
for(const file of ['paths.js','site-config.js','crack-scroll.js','crack-scroll.css','styles.css','story.css','story.js','motion.js','motion.css','scenes.js','ugc-posts.js','ugc-posts.css','commerce.css','catalog.js','commerce.js','app.js'])await fs.copyFile(path.join(root,file),path.join(out,file));
...
function localized(lang,route){let html=source.replace('<div id="scene-story-root"></div>',renderScenes(lang)).replace('<div id="crack-scroll-root"></div>',renderCrack(lang)).replace('<div id="sku-ugc-root"></div>',renderHomepageUgc(lang)).replace('<html lang="ar" dir="rtl">',`<html lang="${lang}" dir="${lang==='ar'?'rtl':'ltr'}">`);
