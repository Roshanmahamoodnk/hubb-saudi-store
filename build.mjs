...
import {products,formats} from './catalog.js';
const root=path.dirname(fileURLToPath(import.meta.url));
const out=path.join(root,'out');
const origin='https://hubb-saudi-gathering-store.r0shan911.chatgpt.site';
await fs.mkdir(path.join(out,'assets'),{recursive:true});
for(const file of ['styles.css','story.css','story.js','commerce.css','catalog.js','commerce.js','app.js'])await fs.copyFile(path.join(root,file),path.join(out,file));
...
function localized(lang,route){let html=source.replace('<html lang="ar" dir="rtl">',`<html lang="${lang}" dir="${lang==='ar'?'rtl':'ltr'}">`);
...
const title=lang==='ar'?'حُبّ | حب دوار الشمس — سنتين لقرمشة تستاهل':'HUBB | Saudi sunflower seeds — Two years. One good crack.';
const description=lang==='ar'?'حُبّ: حب دوار الشمس بأربع نكهات وقصة تطوير سعودية. اكتشف رحلتنا، تسوّق عبوات المشاركة وتواصل معنا للتوزيع والجملة.':'Meet HUBB, Saudi-developed in-shell sunflower seeds in four flavours. Discover our two-year story, explore sharing bundles, and talk wholesale with Rozana.';
