...
<link rel="stylesheet" href="/styles.css"><link rel="stylesheet" href="/commerce.css"><link rel="stylesheet" href="/ugc-posts.css"><link rel="stylesheet" href="/seo-page.css"><link rel="preload" as="image" href="${escape(image)}">
...
<section class="seo-section seo-wrap seo-ugc" aria-label="${escape(tr('منشورات تصوّرية لهذه النكهة', 'Concept posts for this flavour'))}"><div class="seo-section-title"><p class="seo-kicker">${escape(tr('هالنكهة، في لحظة', 'THIS FLAVOUR IN A MOMENT'))}</p><h2>${escape(tr('كما قد تظهر في يومك.', 'As it might show up.'))}</h2><p>${escape(tr('منشوران متخيّلان لكوب الرحلة من هذه النكهة. مشاهد مولّدة بالذكاء الاصطناعي، وليست تقييمات أو منشورات عملاء.', 'Two imagined posts for this flavour’s Journey Cup. AI-generated scenes, not customer reviews or customer posts.'))}</p></div>${renderProductPosts(product.id, lang, { heading: false })}</section>
