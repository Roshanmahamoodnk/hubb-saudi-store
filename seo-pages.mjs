...
function pageHtml({ product, products, formats, onlinePolicy, retailReferencePrice, lang, origin }) {
  ...
  const ugcCards = postsFor(product.id).map(post => `<figure class="seo-ugc-post"><div class="seo-ugc-photo"><img src="${escape(post.image)}" width="1024" height="1536" loading="lazy" alt="${escape(post.alt[lang])}"><span class="seo-ugc-badge">${escape(tr('مشهد تصوّري', 'CONCEPT SCENE'))}</span></div><figcaption><small>${escape(post.place[lang])}</small><p>${escape(post.caption[lang])}</p></figcaption></figure>`).join('');
  const ugcSection = ugcCards ? `<section id="ugc" class="seo-ugc seo-section seo-wrap"><div class="seo-section-title"><p class="seo-kicker">${escape(tr('لحظات نتخيّلها لهذه النكهة', 'MOMENTS WE IMAGINE FOR THIS FLAVOUR'))}</p><h2>${escape(tr(`حُبّ ${name}، في يوم عادي.`, `${name}, in an ordinary day.`))}</h2></div><p class="seo-ugc-note">${escape(tr('مشاهد تخيّلية مولّدة بالذكاء الاصطناعي لهذه النكهة. ليست منشورات عملاء أو تقييمات أو شهادات.', 'AI-generated concept scenes for this flavour. These are not customer posts, reviews or endorsements.'))}</p><div class="seo-ugc-grid">${ugcCards}</div></section>` : '';
  return `<!doctype html>
